    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="md:hidden space-y-3">
        ...
        {!loading && !fetchError && filtered.length === 0 && (
          <p className="text-center text-sm text-muted-foreground py-8">No escalated bills found for the selected filters.</p>
        )}
        {!loading && !fetchError && filtered.map((claim) => {
          const isExpanded = expandedId === claim.upload_id;
          return (
            <Card key={claim.upload_id} className="overflow-hidden">
              <div
                className="cursor-pointer select-none"
                onClick={() => setExpandedId(isExpanded ? null : claim.upload_id)}
              >
                <CardContent className="pt-4 pb-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-sm truncate">{claim.title}</p>
                      <p className="text-xs text-muted-foreground mt-0.5">{claim.submitter_name}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <Badge variant="outline" className={billTypeConfig[claim.usertype].className}>
                        {billTypeConfig[claim.usertype].label}
                      </Badge>
                      <p className="font-mono text-sm font-semibold mt-1">Rs {Number(claim.amount).toLocaleString("en-IN")}</p>
                    </div>
                  </div>
                </CardContent>
              </div>
