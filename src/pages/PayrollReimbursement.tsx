    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="md:hidden space-y-3">
        {loading ? (
          ...
          <>
            {filtered.map((e) => {
              const isExpanded = expandedId === e.reimbursement_id;
              return (
                <Card key={e.reimbursement_id} className="overflow-hidden">
                  <div
                    className="cursor-pointer select-none"
                    onClick={() => setExpandedId(isExpanded ? null : e.reimbursement_id)}
                  >
                    <CardContent className="pt-4 pb-3">
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          <p className="font-medium text-sm truncate">{e.employee_name}</p>
                          <p className="text-xs text-muted-foreground mt-0.5">
                            {e.employee_email ? e.employee_email : "Submitted " + format(parseISO(e.created_at), "dd MMM yyyy")}
                          </p>
                        </div>
                        <div className="text-right shrink-0">
                          <p className="font-mono text-sm font-semibold">₹{Number(e.amount).toLocaleString()}</p>
                          <Badge variant="outline" className={`mt-1 ${statusConfig[e.status]?.className || ""}`}>
                    ...
                    <div className="border-t px-4 py-3 bg-muted/20 space-y-3">
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div>
                          ...
                          <p className="font-medium">{format(parseISO(e.created_at), "dd MMM yyyy")}</p>
                        </div>
                        <div>
                      ...
                      <div className="flex gap-2 pt-1">
                        ...
                        {e.status === "scheduled" ? (
                          ...
                          <span className="text-xs text-muted-foreground self-center flex-1 text-center">Already paid</span>
                        )}
                      </div>
                    </div>
                  )}
            ...
            {filtered.length === 0 && (
              <p className="text-center text-sm text-muted-foreground py-8">No reimbursements match your filters.</p>
            )}
          </>
        )}
      </div>
