  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="md:hidden space-y-3">
        {recordsLoading && (
          <p className="text-center text-sm text-muted-foreground py-10">Loading AR entries...</p>
        )}
        {!recordsLoading && filtered.length === 0 && (
          <p className="text-center text-sm text-muted-foreground py-8">No AR entries match your filters.</p>
        )}
        {!recordsLoading && filtered.map((r) => {
          ...
          const isExpanded = expandedId === r.ar_id;
          return (
            <Card key={r.ar_id} className="overflow-hidden">
              <div
                className="cursor-pointer select-none"
                onClick={() => setExpandedId(isExpanded ? null : r.ar_id)}
              >
                <CardContent className="pt-4 pb-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-sm truncate">{partyNameOf(r)}</p>
                      <p className="text-xs text-muted-foreground capitalize mt-0.5">{r.source_type}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="font-mono text-sm font-semibold">₹{r.amount.toLocaleString()}</p>
                      <Badge variant="outline" className={`mt-1 ${cfg.className}`}>{cfg.label}</Badge>
                    </div>
                  </div>
                </CardContent>
              </div>
                    ...
                    <div className="space-y-1">
                      ...
                      {r.items.map((it, idx) => (
                        <div key={idx} className="flex justify-between text-xs">
                          ...
                          <span className="font-mono font-medium ml-2">₹{it.amount.toLocaleString()}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  {r.employee_name && (
                    <p className="text-xs text-muted-foreground">Employee: {r.employee_name}</p>
                  )}
                  <div className="flex gap-2 pt-1">
                    <button
                      className="h-8 w-8 flex items-center justify-center rounded hover:bg-muted border"
                    ...
                    {isPending ? (
                      ...
                      <span className="text-xs text-muted-foreground self-center flex-1 text-center">No actions available</span>
                    )}
                  </div>
                </div>
              )}
