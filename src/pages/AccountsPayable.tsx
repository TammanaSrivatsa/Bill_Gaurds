    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="md:hidden space-y-3">
        {loading ? (
          ...
          <>
            {filtered.map((e) => {
              const isExpanded = expandedId === e.ap_id;
              return (
                <Card key={e.ap_id} className="overflow-hidden">
                  <div
                    className="cursor-pointer select-none"
                    onClick={() => setExpandedId(isExpanded ? null : e.ap_id)}
                  >
                    <CardContent className="pt-4 pb-3">
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="font-medium text-sm truncate mt-0.5">{e.description}</p>
                        </div>
                        <div className="text-right shrink-0">
                          <p className="font-mono text-sm font-semibold">₹{e.invoice_amount.toLocaleString()}</p>
                          {statusBadge(e)}
                        </div>
                      </div>
                      <div className="flex items-center justify-between mt-2 text-xs text-muted-foreground">
                        ...
                        <span className={daysClass(e)}>{daysLabel(e)}</span>
                      </div>
                    </CardContent>
                  </div>
                  {isExpanded && (
                    <div className="border-t px-4 py-3 bg-muted/20 space-y-2">
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        ...
                        <div>
                          ...
                          <p className={daysClass(e)}>{daysLabel(e)}</p>
                        </div>
                      </div>
                      <div className="flex gap-2 pt-1">
                        {e.status !== "paid" ? (
                          <Button
                            size="sm"
                            className="text-xs w-full bg-primary text-primary-foreground hover:bg-primary/90"
                          ...
                          <span className="text-xs text-muted-foreground w-full text-center py-1.5">Already paid</span>
                        )}
                      </div>
                    </div>
                  )}
            ...
            {filtered.length === 0 && (
              <p className="text-center text-sm text-muted-foreground py-8">No AP entries match your filters.</p>
            )}
          </>
        )}
      </div>
